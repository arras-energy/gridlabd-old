...
int saveall(const char *filename)
{
	...
	if ( ! known_format )
	{
		...
		rc = my_instance->subcommand(converter_command,"%s %s -i %s -o %s %s",(const char*)global_python3_binary,converter_path,input_name,filename,save_options?save_options:"");
		if ( rc != 0 )
		{
