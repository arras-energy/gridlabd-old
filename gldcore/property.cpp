 ...
 * DOUBLE ARRAYS
 *********************************************************/
int double_array_create(void *p)
{
	double_array *a = (double_array*)p;
	int n;
	a->n = a->m = 0;
	a->max = 1;
 ...
 * COMPLEX ARRAYS
 *********************************************************/
int complex_array_create(void *p)
{
	complex_array *a = (complex_array*)p;
	int n;
	a->n = a->m = 0;
	a->max = 1;
...
int string_create(void *p)
{
	memset(p,sizeof(STRING),0);
	return 1;
}
...
{	
	return strcmp( str.buf ? str.buf : "", with);
}
