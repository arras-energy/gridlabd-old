...
STATUS GldCmdarg::no_cmdargs(void)
{
	...
	{
		char command[2048];
		snprintf(command,sizeof(command),std::getenv("GLD_BIN") + std::string("/python3 %s &"),guipath);
		system(command);
		return SUCCESS;
	}
	else
...
int GldCmdarg::library(int argc, const char *argv[])
{
	if ( argc > 1 )
	{
		...
		if ( etcpath == NULL )
		{
			etcpath = strcat(std::getenv("GLD_ETC"),"/gridlabd");
		}
...
int GldCmdarg::_template(int argc, const char *argv[])
{
	if ( argc > 1 )
	{
		...
		snprintf(pathname,sizeof(pathname)-1,"%s/template/%s/%s/%s/%s/%s.glm",getenv("GLD_ETC"),(const char*)global_country,(const char*)global_region,(const char*)global_organization,argv[1],argv[1]);
		return get_instance()->get_loader()->loadall_glm(pathname) == SUCCESS ? 1 : CMDERR;
	}
	else
	{
