...
#undef Z
		...
		if ((use_line_cap == true) && (not_TS_CN == false))
		{
...

			{
...

				{
					...
					{
						warning("Unable to compute capacitance for %s",THISOBJECTHDR->name);
						/* TROUBLESHOOT
...

					{
						...
						if (temp_denom == 0.0)
						{
							warning("Capacitance calculation failure for %s",THISOBJECTHDR->name);
							/*  TROUBLESHOOT
	...
	if(neg_res == true){
		warning("INIT: underground_line:%s has a negative resistance in it's impedance matrix. This will result in unusual behavior. Please check the line's geometry and cable parameters.", obj->name);
		/*  TROUBLESHOOT
