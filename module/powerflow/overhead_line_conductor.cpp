	...
	if(oclass == NULL)
	{
        ...
        if(gl_publish_variable(oclass,
			PT_double, "geometric_mean_radius[ft]",PADDR(geometric_mean_radius),
				PT_REQUIRED,
				PT_DESCRIPTION, "radius of the conductor",
			PT_double, "resistance[Ohm/mile]",PADDR(resistance),
				PT_REQUIRED,
				PT_DESCRIPTION, "resistance in Ohms/mile of the conductor",
			PT_double, "diameter[in]",PADDR(cable_diameter),
				PT_REQUIRED,
				PT_DESCRIPTION, "Diameter of line for capacitance calculations",
			PT_double, "rating.summer.continuous[A]", PADDR(summer.continuous),
				PT_DEFAULT, "1000 A",
				PT_DESCRIPTION, "Continuous summer amp rating",
			PT_double, "rating.summer.emergency[A]", PADDR(summer.emergency),
				PT_DEFAULT, "2000 A",
				PT_DESCRIPTION, "Emergency summer amp rating",
			PT_double, "rating.winter.continuous[A]", PADDR(winter.continuous),
				PT_DEFAULT, "1000 A",
				PT_DESCRIPTION, "Continuous winter amp rating",
...
int overhead_line_conductor::init(OBJECT *parent)
{
	...
	if (resistance == 0.0)
	{
		...
		{
			warning("overhead_line_conductor:%d - %s - FBS: resistance is zero",get_id(),get_name());
			/*  TROUBLESHOOT
			The overhead_line_conductor has a resistance of zero.  This will cause problems with the 
