	...
	if (oclass==NULL)
	{
		...
		if (gl_publish_variable(oclass,
			...
			PT_object,"configuration",PADDR(configuration),
				PT_REQUIRED,
				PT_DESCRIPTION,"reference to the regulator_configuration object used to determine regulator properties",
			PT_int16, "tap_A",PADDR(tap_A),
				PT_DESCRIPTION,"current tap position of tap A",
			PT_int16, "tap_B",PADDR(tap_B),
				PT_DESCRIPTION,"current tap position of tap B",
			PT_int16, "tap_C",PADDR(tap_C),
				PT_DESCRIPTION,"current tap position of tap C",
			PT_double, "tap_A_change_count",PADDR(tap_A_change_count),
				PT_OUTPUT,
				PT_DESCRIPTION,"count of all physical tap changes on phase A since beginning of simulation (plus initial value)",
			PT_double, "tap_B_change_count",PADDR(tap_B_change_count),
				PT_OUTPUT,
				PT_DESCRIPTION,"count of all physical tap changes on phase B since beginning of simulation (plus initial value)",
			PT_double, "tap_C_change_count",PADDR(tap_C_change_count),
				PT_OUTPUT,
				PT_DESCRIPTION,"count of all physical tap changes on phase C since beginning of simulation (plus initial value)",
			PT_object,"sense_node",PADDR(RemoteNode),
				PT_DESCRIPTION,"Node to be monitored for voltage control in remote sense mode",
			PT_double,"regulator_resistance[Ohm]",PADDR(regulator_resistance),
				PT_DESCRIPTION,"The resistance value of the regulator when it is not blown.",
