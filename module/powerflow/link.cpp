	...
	if (oclass==NULL)
	{
		...
		if(gl_publish_variable(oclass,
			...
			PT_object, "to", PADDR(to),
				PT_REQUIRED,
				PT_DESCRIPTION,"to_node - load node",

			PT_complex, "power_in[VA]", PADDR(power_in),
				PT_OUTPUT,
				PT_DESCRIPTION,"power flow in (w.r.t from node)",
			PT_complex, "power_out[VA]", PADDR(power_out),
				PT_OUTPUT,
				PT_DESCRIPTION,"power flow out (w.r.t to node)",
			PT_double, "power_out_real[W]", PADDR(power_out.Re()),
				PT_OUTPUT,
				PT_DESCRIPTION,"power flow out (w.r.t to node), real",
			PT_complex, "power_losses[VA]", PADDR(power_loss),
				PT_OUTPUT,
				PT_DESCRIPTION,"power losses",
			PT_complex, "power_in_A[VA]", PADDR(indiv_power_in[0]),
				PT_OUTPUT,
				PT_DESCRIPTION,"power flow in (w.r.t from node), phase A",
			PT_complex, "power_in_B[VA]", PADDR(indiv_power_in[1]),
				PT_OUTPUT,
				PT_DESCRIPTION,"power flow in (w.r.t from node), phase B",
			PT_complex, "power_in_C[VA]", PADDR(indiv_power_in[2]),
				PT_OUTPUT,
				PT_DESCRIPTION,"power flow in (w.r.t from node), phase C",
			PT_complex, "power_out_A[VA]", PADDR(indiv_power_out[0]),
				PT_OUTPUT,
				PT_DESCRIPTION,"power flow out (w.r.t to node), phase A",
			PT_complex, "power_out_B[VA]", PADDR(indiv_power_out[1]),
				PT_OUTPUT,
				PT_DESCRIPTION,"power flow out (w.r.t to node), phase B",
			PT_complex, "power_out_C[VA]", PADDR(indiv_power_out[2]),
				PT_OUTPUT,
				PT_DESCRIPTION,"power flow out (w.r.t to node), phase C",
			PT_complex, "power_losses_A[VA]", PADDR(indiv_power_loss[0]),
				PT_OUTPUT,
				PT_DESCRIPTION,"power losses, phase A",
			PT_complex, "power_losses_B[VA]", PADDR(indiv_power_loss[1]),
				PT_OUTPUT,
				PT_DESCRIPTION,"power losses, phase B",
			PT_complex, "power_losses_C[VA]", PADDR(indiv_power_loss[2]),
				PT_OUTPUT,
				PT_DESCRIPTION,"power losses, phase C",
			PT_complex, "current_out_A[A]", PADDR(read_I_out[0]),
				PT_OUTPUT,
				PT_DESCRIPTION,"current flow out of link (w.r.t. to node), phase A",
			PT_complex, "current_out_B[A]", PADDR(read_I_out[1]),
				PT_OUTPUT,
				PT_DESCRIPTION,"current flow out of link (w.r.t. to node), phase B",
			PT_complex, "current_out_C[A]", PADDR(read_I_out[2]),
				PT_OUTPUT,
				PT_DESCRIPTION,"current flow out of link (w.r.t. to node), phase C",
			PT_complex, "current_in_A[A]", PADDR(read_I_in[0]),
				PT_OUTPUT,
				PT_DESCRIPTION,"current flow to link (w.r.t from node), phase A",
			PT_complex, "current_in_B[A]", PADDR(read_I_in[1]),
				PT_OUTPUT,
				PT_DESCRIPTION,"current flow to link (w.r.t from node), phase B",
			PT_complex, "current_in_C[A]", PADDR(read_I_in[2]),
				PT_OUTPUT,
				PT_DESCRIPTION,"current flow to link (w.r.t from node), phase C",
			PT_complex, "fault_current_in_A[A]", PADDR(If_in[0]),
				PT_OUTPUT,
				PT_DESCRIPTION,"fault current flowing in, phase A",
			PT_complex, "fault_current_in_B[A]", PADDR(If_in[1]),
				PT_OUTPUT,
				PT_DESCRIPTION,"fault current flowing in, phase B",
			PT_complex, "fault_current_in_C[A]", PADDR(If_in[2]),
				PT_OUTPUT,
				PT_DESCRIPTION,"fault current flowing in, phase C",
			PT_complex, "fault_current_out_A[A]", PADDR(If_out[0]),
				PT_OUTPUT,
				PT_DESCRIPTION,"fault current flowing out, phase A",
			PT_complex, "fault_current_out_B[A]", PADDR(If_out[1]),
				PT_OUTPUT,
				PT_DESCRIPTION,"fault current flowing out, phase B",
			PT_complex, "fault_current_out_C[A]", PADDR(If_out[2]),
				PT_OUTPUT,
				PT_DESCRIPTION,"fault current flowing out, phase C",

				PT_OUTPUT,
			PT_complex, "fault_voltage_A[A]", PADDR(Vf_out[0]),
				PT_DESCRIPTION,"fault voltage, phase A",
			PT_complex, "fault_voltage_B[A]", PADDR(Vf_out[1]),
				PT_DESCRIPTION,"fault voltage, phase B",
			PT_complex, "fault_voltage_C[A]", PADDR(Vf_out[2]),
				PT_DESCRIPTION,"fault voltage, phase C",
				
			PT_set, "flow_direction", PADDR(flow_direction),
				PT_OUTPUT,
				PT_DESCRIPTION,"flag used for describing direction of the flow of power",
			...
			PT_double, "continuous_rating[A]", PADDR(link_rating[0]),
				PT_DESCRIPTION, "Continuous rating for this link object (set individual line segments",
			PT_double, "emergency_rating[A]", PADDR(link_rating[1]),
