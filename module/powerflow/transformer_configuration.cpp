...
transformer_configuration::transformer_configuration(MODULE *mod) : powerflow_library(mod)
{
	if(oclass == NULL)
	{
		oclass = gl_register_class(mod,"transformer_configuration",sizeof(transformer_configuration),PC_NOSYNC);
		if (oclass==NULL)
		...
		if(gl_publish_variable(oclass,
			PT_enumeration,"connect_type",PADDR(connect_type),
				...
				PT_DEFAULT, "UNKNOWN",
				PT_DESCRIPTION, "transformation connection type",
			...
			PT_enumeration,"install_type",PADDR(install_type),
				PT_DESCRIPTION,"Defines location of the transformer installation",
				PT_DEFAULT, "UNKNOWN",
				PT_DESCRIPTION, "transformation installation type",
				PT_KEYWORD,"UNKNOWN",UNKNOWN,
			...
			PT_enumeration,"coolant_type", PADDR(coolant_type),
				PT_DESCRIPTION,"coolant type, used in life time model",
				PT_DEFAULT, "UNKNOWN",
				PT_DESCRIPTION, "transformation coolant type",
				PT_KEYWORD,"UNKNOWN", UNKNOWN,
			...
			PT_enumeration, "cooling_type", PADDR(cooling_type),
				PT_DESCRIPTION,"type of coolant fluid used in life time model",
				PT_DEFAULT, "UNKNOWN",
				PT_DESCRIPTION, "transformation cooling type",
				PT_KEYWORD,"UNKNOWN", UNKNOWN,
			...
			PT_double, "primary_voltage[V]", PADDR(V_primary),
				PT_REQUIRED,
				PT_DESCRIPTION,"primary voltage level in L-L value kV",
			PT_double, "secondary_voltage[V]",PADDR(V_secondary),
				PT_REQUIRED,
				PT_DESCRIPTION,"secondary voltage level kV",
			PT_double, "power_rating[kVA]",PADDR(kVA_rating),
				PT_REQUIRED,
				PT_DESCRIPTION,"kVA rating of transformer, total",
			PT_double, "powerA_rating[kVA]",PADDR(phaseA_kVA_rating),
				PT_DESCRIPTION,"kVA rating of transformer, phase A",
			PT_double, "powerB_rating[kVA]",PADDR(phaseB_kVA_rating),
				PT_DESCRIPTION,"kVA rating of transformer, phase B",
			PT_double, "powerC_rating[kVA]",PADDR(phaseC_kVA_rating),
				PT_DESCRIPTION,"kVA rating of transformer, phase C",
			PT_double, "powerN_rating[kVA]",PADDR(phaseN_kVA_rating),
				PT_DESCRIPTION,"kVA rating of transformer, neutral",
			PT_double, "resistance[pu*Ohm]",PADDR(impedance.Re()),
				PT_DESCRIPTION,"Series impedance, pu, real",
			PT_double, "reactance[pu*Ohm]",PADDR(impedance.Im()),
				PT_DESCRIPTION,"Series impedance, pu, imag",
			PT_complex, "impedance[pu*Ohm]",PADDR(impedance),
				PT_DESCRIPTION,"Series impedance, pu",
			PT_double, "resistance1[pu*Ohm]",PADDR(impedance1.Re()),
				PT_DESCRIPTION,"Secondary series impedance (only used when you want to define each individual winding seperately, pu, real",
			PT_double, "reactance1[pu*Ohm]",PADDR(impedance1.Im()),
				PT_DESCRIPTION,"Secondary series impedance (only used when you want to define each individual winding seperately, pu, imag",
			PT_complex, "impedance1[pu*Ohm]",PADDR(impedance1),
				PT_DESCRIPTION,"Secondary series impedance (only used when you want to define each individual winding seperately, pu",
			...
			PT_complex, "shunt_impedance[pu*Ohm]",PADDR(shunt_impedance),
				PT_DEFAULT,"+1e+09+1e+09j pu*Ohm",
			...
			PT_double, "core_coil_weight[lb]", PADDR(core_coil_weight),
				PT_DESCRIPTION,"The weight of the core and coil assembly in pounds",
			...
			PT_double, "oil_volume[gal]", PADDR(oil_vol),
				PT_DESCRIPTION,"The number of gallons of oil in the transformer",
			PT_double, "rated_winding_time_constant[h]", PADDR(t_W),
				PT_DESCRIPTION,"The rated winding time constant in hours",
			PT_double, "rated_winding_hot_spot_rise[degC]", PADDR(dtheta_H_AR),
				PT_DESCRIPTION,"winding hottest-spot rise over ambient temperature at rated load, degrees C",
			PT_double, "rated_top_oil_rise[degC]", PADDR(dtheta_TO_R),
				PT_DESCRIPTION,"top-oil hottest-spot rise over ambient temperature at rated load, degrees C",
			PT_double, "no_load_loss[pu]", PADDR(no_load_loss),
				PT_DESCRIPTION,"Another method of specifying transformer impedances, defined as per unit power values (shunt)",
			PT_double, "full_load_loss[pu]", PADDR(full_load_loss),
				PT_DESCRIPTION,"Another method of specifying transformer impedances, defined as per unit power values (shunt and series)",
			PT_double, "reactance_resistance_ratio", PADDR(RX),
				PT_DESCRIPTION,"the reactance to resistance ratio (X/R)",
			PT_double, "installed_insulation_life[h]", PADDR(installed_insulation_life),
				PT_DESCRIPTION,"the normal lifetime of the transformer insulation at rated load, hours",

			//In-rush/saturation parameters
			PT_enumeration,"magnetization_location",PADDR(magnetization_location),
				PT_DESCRIPTION,"winding to place magnetization influence for in-rush calculations",
			...
			PT_bool, "inrush_saturation_enabled", PADDR(model_inrush_saturation),
				PT_DESCRIPTION,"flag to include saturation effects during inrush calculations",
			PT_double, "L_A[pu]", PADDR(LA_pu),
				PT_DEFAULT,"+0.2 pu",
				PT_DESCRIPTION,"Air core inductance of transformer",
			PT_double, "phi_K[pu]", PADDR(phiK_pu),
				PT_DEFAULT,"+1.17 pu",
				PT_DESCRIPTION,"Knee flux value where the air core inductance interstes the flux axis of the saturation curve",
			PT_double, "phi_M[pu]", PADDR(phiM_pu),
				PT_DEFAULT,"+1 pu",
				PT_DESCRIPTION,"Peak magnetization flux at rated voltage of the saturation curve",
			PT_double, "I_M[pu]", PADDR(IM_pu),
				PT_DEFAULT,"+0.01 pu",
				PT_DESCRIPTION,"Peak magnetization current at rated voltage of the saturation curve",
			PT_double, "T_D", PADDR(TD_val),
				PT_DEFAULT,"+0.5",
				PT_DESCRIPTION, "Inrush decay time constant for inrush current",
