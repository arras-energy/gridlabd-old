	...
	if (oclass==NULL)
	{
		...
		if (gl_publish_variable(oclass,
			PT_enumeration,"generator_mode",PADDR(gen_mode_v), 
				PT_DEFAULT,"UNKNOWN", 
				PT_DESCRIPTION, "LEGACY MODEL: Selects generator control mode when using legacy model; in non-legacy models, this should be SUPPLY_DRIVEN.",
			...
			PT_enumeration,"generator_status",PADDR(gen_status_v),  
				PT_DEFAULT,"ONLINE",  
				PT_DESCRIPTION, "describes whether the generator is online or offline",
				PT_KEYWORD,"OFFLINE",(enumeration)OFFLINE,
				PT_KEYWORD,"ONLINE",(enumeration)ONLINE,	

			PT_enumeration,"rfb_size", PADDR(rfb_size_v),  
				PT_DESCRIPTION, "Default settings for certain sizes of batteries",
			...
			PT_enumeration,"power_type",PADDR(power_type_v),  
				PT_DESCRIPTION, "LEGACY MODEL: Not currently used", //This is not used anywhere in the code. I would suggest removing it from the model.
			...
			PT_double, "number_battery_state_changes", PADDR(no_of_cycles), 
				PT_DESCRIPTION, "LEGACY MODEL: Number of times battery switches between charging and discharging", 

			// Used for linear temperature control mode
			PT_double, "monitored_power[W]", PADDR(check_power),  
				PT_DESCRIPTION, "LEGACY MODEL: output only; power output value of parent meter when performing load following modes (POWER_DRIVEN)",
			PT_double, "power_set_high[W]", PADDR(power_set_high),  
				PT_DESCRIPTION, "LEGACY MODEL: high set point of dead band for load following (POWER_DRIVEN)",
			PT_double, "power_set_low[W]", PADDR(power_set_low),  
				PT_DESCRIPTION, "LEGACY MODEL: low set point of dead band for load following (POWER_DRIVEN)",
			PT_double, "power_set_high_highT[W]", PADDR(power_set_high_highT),  
				PT_DESCRIPTION, "LEGACY MODEL: high set point of dead band for load following at higher temperatures (POWER_DRIVEN + LINEAR_TEMPERATURE)",
			PT_double, "power_set_low_highT[W]", PADDR(power_set_low_highT),  
				PT_DESCRIPTION, "LEGACY MODEL: low set point of dead band for load following at higher temperatures (POWER_DRIVEN + LINEAR_TEMPERATURE)",
			PT_double, "check_power_low[W]", PADDR(check_power_low),  
				PT_DESCRIPTION, "LEGACY MODEL: high set point of dead band for load following at lower temperatures (POWER_DRIVEN + LINEAR_TEMPERATURE)",
			PT_double, "check_power_high[W]", PADDR(check_power_high),  
				PT_DESCRIPTION, "LEGACY MODEL: low set point of dead band for load following at lower temperatures (POWER_DRIVEN + LINEAR_TEMPERATURE)",
			...
			PT_double, "deadband[V]", PADDR(deadband), PT_DEFAULT,"0 V",  
				PT_DESCRIPTION, "LEGACY MODEL: voltage deadband",
			PT_double, "sensitivity", PADDR(sensitivity),  
				PT_DESCRIPTION, "LEGACY MODEL: describes how sensitive the control is to temperature excursions; defines slope of linear control",
			PT_double, "high_temperature[degF]", PADDR(high_temperature),  
				PT_DEFAULT,"100 degF",  
				PT_DESCRIPTION, "LEGACY MODEL: high temperature of linear control; defines slope",
			PT_double, "midpoint_temperature[degF]", PADDR(midpoint_temperature),  
				PT_DEFAULT,"50 degF",  
				PT_DESCRIPTION, "LEGACY MODEL: midpoint temperature of linear control; defines slope",
			PT_double, "low_temperature[degF]", PADDR(low_temperature),  
				PT_DEFAULT,"0 degF",  
				PT_DESCRIPTION, "LEGACY MODEL: low temperature of linear control; defines slope",

			//Used for PQ mode
			PT_double, "scheduled_power[W]", PADDR(B_scheduled_power),  
				PT_DESCRIPTION, "LEGACY MODEL: real power output of battery/inverter system",

			PT_double, "Rinternal[Ohm]", PADDR(Rinternal),  
				PT_DEFAULT, "10 Ohm",  
				PT_DESCRIPTION, "LEGACY MODEL: the internal resistance of the battery.",
			PT_double, "V_Max[V]", PADDR(V_Max),  
				PT_DEFAULT, "0 V",  
				PT_DESCRIPTION, "LEGACY MODEL: the maximum terminal voltage of the battery.",
			PT_complex, "I_Max[A]", PADDR(I_Max),  
				PT_DEFAULT, "0 A",  
				PT_DESCRIPTION, "LEGACY MODEL: the maximum current output of the battery.",
			PT_double, "E_Max[Wh]", PADDR(E_Max),  
				PT_DEFAULT, "0 Wh",  
				PT_DESCRIPTION, "LEGACY MODEL: the maximum capacity of the battery.",
			PT_double, "P_Max[W]", PADDR(Max_P),  
				PT_DEFAULT, "0 W",  
				PT_DESCRIPTION, "LEGACY MODEL: the maximum power output of the battery.",
			PT_double, "power_factor", PADDR(pf),  
				PT_DESCRIPTION, "LEGACY MODEL: the power factor output of the battery.",
			PT_double, "Energy[Wh]",PADDR(Energy),  
				PT_DEFAULT, "-1",  
				PT_DESCRIPTION, "LEGACY MODEL: the available capacity of the battery.",
			PT_double, "efficiency[unit]", PADDR(efficiency),  
				PT_DEFAULT,"0 unit",  
				PT_DESCRIPTION, "LEGACY MODEL: the efficiency of the battery.",
			PT_double, "base_efficiency[unit]", PADDR(base_efficiency),  
				PT_DEFAULT,"0 unit",  
				PT_DESCRIPTION, "LEGACY MODEL: the efficiency of the battery at rated voltaged and current.",
			...
			PT_complex, "I_Out[A]", PADDR(I_Out),  
				PT_DESCRIPTION, "LEGACY MODEL: the AC current output of the battery.",
			PT_complex, "VA_Out[VA]", PADDR(VA_Out),  
				PT_DESCRIPTION, "LEGACY MODEL: the power output of the battery.",
			PT_complex, "V_In[V]", PADDR(V_In),  
				PT_DESCRIPTION, "LEGACY MODEL: the voltage at the terminals of the battery.",
			PT_complex, "I_In[A]", PADDR(I_In),  
				PT_DESCRIPTION, "LEGACY MODEL: the current flowing into the battery of the battery.",
			PT_complex, "V_Internal[V]", PADDR(V_Internal),  
				PT_DESCRIPTION, "LEGACY MODEL: the internal voltage of the battery.",
			PT_complex, "I_Internal[A]",PADDR(I_Internal),  
				PT_DESCRIPTION, "LEGACY MODEL: the internal current of the battery.",
			PT_complex, "I_Prev[A]", PADDR(I_Prev),  
				PT_DESCRIPTION, "LEGACY MODEL: the previous current output of the battery.",
			...
			PT_bool,"use_internal_battery_model", PADDR(use_internal_battery_model),  
				PT_DEFAULT,"FALSE",  
				PT_DESCRIPTION, "Enables the INTERNAL BATTERY MODEL.",
			PT_enumeration,"battery_type", PADDR(battery_type),  
				PT_DESCRIPTION, "INTERNAL BATTERY MODEL: the type of the battery. Used to determine the soc vs voltage curve.",
			...
			PT_double,"nominal_voltage[V]", PADDR(v_max),  
				PT_DESCRIPTION, "INTERNAL BATTERY MODEL: the rated DC voltage at the terminals of the battery.",
			PT_double,"rated_power[W]", PADDR(p_max),  
				PT_DESCRIPTION, "INTERNAL BATTERY MODEL: the rated power output of the battery.",
			PT_double,"battery_capacity[Wh]", PADDR(e_max),  
				PT_DESCRIPTION, "INTERNAL BATTERY MODEL: the rated battery capacity of the battery.",
			PT_double,"round_trip_efficiency[pu]", PADDR(eta_rt),  
				PT_DESCRIPTION, "INTERNAL BATTERY MODEL: the round trip efficiency of the battery according to a full discharge/charge cycle.",
			PT_double,"state_of_charge[pu]", PADDR(soc),  
				PT_DEFAULT,"-1 pu",  
				PT_DESCRIPTION, "INTERNAL BATTERY MODEL: the current state of charge of the battery.",
			PT_double,"battery_load[W]", PADDR(bat_load),  
				PT_DESCRIPTION, "INTERNAL BATTERY MODEL: the current power output of the battery.",
