...
climate::climate(MODULE *module) : gld_object()
{
	...
	if (oclass==NULL)
	{
		...
		if (gl_publish_variable(oclass,
			PT_double,"solar_elevation[rad]",PADDR(solar_elevation), 
				PT_OUTPUT, 
				PT_DESCRIPTION,"solar elevation angle in radians",
			PT_double,"solar_azimuth[rad]",PADDR(solar_azimuth), 
				PT_OUTPUT, 
				PT_DESCRIPTION,"solar azimuth angle in radians",
			...
			PT_char1024,"tmyfile",PADDR(tmyfile), 
				PT_REQUIRED, 
				PT_DESCRIPTION,"weather data file name",
			PT_double,"temperature[degF]",PADDR(temperature),
				PT_OUTPUT, 
				PT_DEFAULT,"59.0 degF", 
				PT_DESCRIPTION,"current temperature",
			PT_double,"humidity[pu]",PADDR(humidity), 
				PT_OUTPUT, 
				PT_DEFAULT,"75%", 
				PT_DESCRIPTION,"current humidity",
			PT_double,"solar_flux[W/sf]",PADDR(solar_flux),	PT_SIZE,9, 
				PT_OUTPUT, 
				PT_DESCRIPTION,"current solar irradiance (9 orientiations)",
			PT_double,"solar_direct[W/sf]",PADDR(solar_direct), 
				PT_OUTPUT, 
				PT_DESCRIPTION,"solar direct irradiance",
			PT_double,"solar_diffuse[W/sf]",PADDR(solar_diffuse), 
				PT_OUTPUT, 
				PT_DESCRIPTION,"solar diffuse irradiance",
			PT_double,"solar_global[W/sf]",PADDR(solar_global), 
				PT_OUTPUT, 
				PT_DESCRIPTION,"solar global flux irradiance",
			PT_double,"extraterrestrial_global_horizontal[W/sf]",PADDR(global_horizontal_extra), 
				PT_OUTPUT, 
				PT_DESCRIPTION,"solar global extraterrestrial irradiance",
			PT_double,"extraterrestrial_direct_normal[W/sf]",PADDR(direct_normal_extra), 
				PT_OUTPUT, 
				PT_DEFAULT,"1367 W/m^2", 
				PT_DESCRIPTION,"solar direct normal extraterrestrial irradiance",
			PT_double,"pressure[mbar]",PADDR(pressure), 
				PT_OUTPUT, 
				PT_DEFAULT,"1013.25 mbar",  
				PT_DESCRIPTION,"current air pressure",
			PT_double,"wind_speed[m/s]", PADDR(wind_speed), 
				PT_OUTPUT, 
				PT_DESCRIPTION,"current wind speed",
			PT_double,"wind_dir[rad]", PADDR(wind_dir), 
				PT_OUTPUT, 
				PT_DESCRIPTION,"current direction in radians",
			PT_double,"wind_gust[m/s]", PADDR(wind_gust), 
				PT_OUTPUT, 
				PT_DESCRIPTION,"current wind gusts",
			PT_double,"record.low[degF]", PADDR(record.low), 
				PT_OUTPUT, 
				PT_DESCRIPTION,"record low temperature observed",
			PT_int32,"record.low_day",PADDR(record.low_day), 
				PT_OUTPUT, 
				PT_DESCRIPTION,"day of year for record low observation",
			PT_double,"record.high[degF]", PADDR(record.high), 
				PT_OUTPUT, 
				PT_DESCRIPTION,"record high temperature observed",
			PT_int32,"record.high_day",PADDR(record.high_day), 
				PT_OUTPUT, 
				PT_DESCRIPTION,"day of year for record high observation",
			PT_double,"record.solar[W/sf]", PADDR(record.solar), 
				PT_OUTPUT, 
				PT_DESCRIPTION,"record high solar irradiance observed",
			PT_double,"rainfall[in/h]",PADDR(rainfall), 
				PT_OUTPUT, 
				PT_DESCRIPTION,"rainfall observed",
			PT_double,"snowdepth[in]",PADDR(snowdepth), 
				PT_OUTPUT, 
				PT_DESCRIPTION,"snow depth observed",
			PT_enumeration,"interpolate",PADDR(interpolate), 
				PT_OUTPUT, 
				PT_DEFAULT,"NONE", 
			...
			PT_double,"solar_horiz",PADDR(solar_flux[CP_H]), 
				PT_OUTPUT, 
				PT_DESCRIPTION, "solar flux on horizontal surface",
			PT_double,"solar_north",PADDR(solar_flux[CP_N]), 
				PT_OUTPUT, 
				PT_DESCRIPTION, "solar flux on north surface",
			PT_double,"solar_northeast",PADDR(solar_flux[CP_NE]), 
				PT_OUTPUT, 
				PT_DESCRIPTION, "solar flux on northeast surface",
			PT_double,"solar_east",PADDR(solar_flux[CP_E]),
				PT_OUTPUT,
				PT_DESCRIPTION, "solar flux on east surface",
			PT_double,"solar_southeast",PADDR(solar_flux[CP_SE]),
				PT_OUTPUT,
				PT_DESCRIPTION, "solar flux on southeast surface",
			PT_double,"solar_south",PADDR(solar_flux[CP_S]),
				PT_OUTPUT,
				PT_DESCRIPTION, "solar flux on south surface",
			PT_double,"solar_southwest",PADDR(solar_flux[CP_SW]),
				PT_OUTPUT,
				PT_DESCRIPTION, "solar flux on southwest surface",
			PT_double,"solar_west",PADDR(solar_flux[CP_W]),
				PT_OUTPUT,
				PT_DESCRIPTION, "solar flux on west surface",
			PT_double,"solar_northwest",PADDR(solar_flux[CP_NW]),
				PT_OUTPUT,
				PT_DESCRIPTION, "solar flux on northwest surface",
			PT_double,"solar_raw[W/sf]",PADDR(solar_raw),
				PT_OUTPUT,
				PT_DESCRIPTION, "raw solar flux",
			PT_double,"ground_reflectivity[pu]",PADDR(ground_reflectivity),
				PT_OUTPUT,
				PT_DEFAULT,"0.3 pu",
				PT_DESCRIPTION,"ground reflectivity observed",
			PT_object,"reader",PADDR(reader),
				PT_DESCRIPTION,"weather reader object",
			PT_char1024,"forecast",PADDR(forecast_spec),
				PT_DESCRIPTION,"forecasting specifications", 
			PT_enumeration,"cloud_model",PADDR(cloud_model),
				PT_OUTPUT,
				PT_DEFAULT, "NONE",
				PT_DESCRIPTION,"the cloud model to use",
