...
csv_reader::csv_reader(MODULE *module)
{
	...
	if (oclass==NULL)
	{
		...
		if (gl_publish_variable(oclass,
			PT_int32,"index",PADDR(index),PT_ACCESS,PA_REFERENCE, PT_DESCRIPTION, "reader line index",
			PT_char32,"city_name",PADDR(city_name), PT_DESCRIPTION, "name of city",
			PT_char32,"state_name",PADDR(state_name), PT_DESCRIPTION, "name of state/province",
			PT_double,"lat_deg",PADDR(lat_deg), PT_DESCRIPTION, "latitude degrees",
			PT_double,"lat_min",PADDR(lat_min), PT_DESCRIPTION, "latitude minutes",
			PT_double,"long_deg", PADDR(long_deg), PT_DESCRIPTION, "longitude degrees",
			PT_double,"long_min",PADDR(long_min), PT_DESCRIPTION, "longitude minutes",
			PT_double,"low_temp",PADDR(low_temp),PT_ACCESS,PA_REFERENCE, PT_DESCRIPTION, "lowest temperature",
			PT_double,"high_temp",PADDR(high_temp),PT_ACCESS,PA_REFERENCE, PT_DESCRIPTION, "highest temperature",
			PT_double,"peak_solar",PADDR(peak_solar),PT_ACCESS,PA_REFERENCE, PT_DESCRIPTION, "peak solar flux",
			PT_int32,"elevation",PADDR(elevation), PT_DESCRIPTION, "elevation",
			PT_enumeration,"status",PADDR(status),PT_ACCESS,PA_REFERENCE, PT_DESCRIPTION, "reader status",
				PT_KEYWORD,"INIT",(enumeration)CR_INIT,
				PT_KEYWORD,"OPEN",(enumeration)CR_OPEN,
				PT_KEYWORD,"ERROR",(enumeration)CR_ERROR,
			PT_char32,"timefmt",PADDR(timefmt), PT_DESCRIPTION, "time format",
			PT_char32,"timezone",PADDR(timezone), PT_DESCRIPTION, "timezone",
			PT_double,"timezone_offset",PADDR(tz_numval), PT_DESCRIPTION, "timezone offset",
