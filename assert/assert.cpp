...
g_assert::g_assert(MODULE *module)
{
	if (oclass==NULL)
	{
		...
		if (gl_publish_variable(oclass,
			...
			PT_char1024, "target", get_target_offset(), PT_REQUIRED, PT_DESCRIPTION, "the target property to test",
			PT_char32, "part", get_part_offset(), PT_DESCRIPTION, "the target property part to test",
			PT_enumeration,"relation",get_relation_offset(), PT_REQUIRED, PT_DESCRIPTION, "the relation to use for the test",
			...
			PT_char1024, "value", get_value_offset(), PT_REQUIRED, PT_DESCRIPTION, "the value to compare with for binary tests",
