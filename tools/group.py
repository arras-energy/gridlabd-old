...
is JSON. When `modify` is used, the output file format is GLM, and two python
...
control objects and which groups are connected by that object.
		...
		with open(OUTPUT,"w") as fh:
			if MODIFY:
				...
				for obj,data in model['objects'].items():
					...
					if groupid == CONTROL:
						if not obj in controls:
							controls[obj] = groupid
						else:
							controls[obj].append(groupid)
