...
"""
    L = len(Y)
    M = np.hstack([np.hstack([Y[n:L - K + n] for n in range(1, K + 1)]),
                   np.hstack([X[n:L - K + n] for n in range(K + 1)])])
    x = np.linalg.lstsq(M, Y[:-K])[0]
...
if __name__ == '__main__':
    ...
    def syntax(code=0):
        if code == 0:
            ...
            print(
                f"Syntax: {basename} -o|--output=OUTPUTCSV -i|--input=INPUTCSV [-g|--glm=GLMNAME] [-n|--name=LOADNAME] [-t|--type=LOADTYPE] [--test] [-h|--help|help] [OPTIONS ...]")
        exit(code)
    ...
    class converters:
        ...
        outputs = {"START_TIME_PST": datetime, 'LMP': real}
        inputs = {"START_TIME_PST": datetime, 'MW': integer}
    ...
    if inputs and outputs:
        ...
        for output_name in output_names:
            ...
            try:
                ...
                # Uncomment to view plot of train vs test
                ...
                x_train, M_train = identify(Y_train, U_train, K)

                # Calculate x vector and M for test set
                x_test, M_test = identify(Y_test, U_test, K)
