...
def convert_timezone(market, date_str):
    ...
    if market == 'caiso':
        ...
        local_dt = local_tz.localize(naive_dt,
                                     is_dst=None)  # Setting to none will raise errors for ambiguous or non-existent times
...
def get_dates(start_date_str, end_date_str, max_interval):
    ...
    while (end_date_dt - start_date_temp_dt).days > 0:
        ...
        start_date_temp_str = format_date(market, convert_timezone(market, start_date_temp_dt.strftime('%Y%m%d')))
        end_date_temp_str = format_date(market, convert_timezone(market, end_date_temp_dt.strftime('%Y%m%d')))
        dates_list.append({'start_date_str': start_date_temp_str, 'end_date_str': end_date_temp_str})
...
def get_caiso_data(node, start_date, end_date):
    ...
    lmp_folder = create_folder(data_type='lmp', market='caiso', start_date=start_date, end_date=end_date)
    demand_folder = create_folder(data_type='demand', market='caiso', start_date=start_date, end_date=end_date)
    ...
    dates_list = get_dates(start_date_str=start_date, end_date_str=end_date, max_interval=31)
    ...
    print(f'Process complete. Data saved in output/caiso_<data_type>_{start_date}-{end_date}.')
...
def get_isone_data(node, start_date, end_date):
    ...
    lmp_folder = create_folder(data_type='lmp', market='isone', start_date=start_date, end_date=end_date)
    demand_folder = create_folder(data_type='demand', market='isone', start_date=start_date, end_date=end_date)
    ...
    while start_date_dt <= end_date_dt:
        # Get LMP data
        endpoint = f"/fiveminutelmp/day/{start_date_dt.strftime('%Y%m%d')}/location/{node}"
        response = requests.get(base_url + endpoint + file_type, auth=HTTPBasicAuth(credentials_dict['user'], credentials_dict['pwd']))
        response_dict = response.json()['FiveMinLmps']
        lmp_df = pd.concat([pd.DataFrame(v) for k, v in response_dict.items()], keys=response_dict)
        lmp_df.to_csv(f"{lmp_folder}/isone_fiveminlmp_{start_date_dt.strftime('%Y%m%d')}.csv")
        ...
        demand_df = pd.concat([pd.DataFrame(v) for k, v in response_dict.items()], keys=response_dict)
        demand_df.to_csv(f"{demand_folder}/isone_fivemindemand_{start_date_dt.strftime('%Y%m%d')}.csv")

        # Run again for the next day in range
        start_date_dt += dt.timedelta(days=1)

    print(f'Process complete. Data saved in output/isone_<data_type>_{start_date}-{end_date}.')
...
def syntax(code=0):
    ...
    if code == 0:
        ...
        print(f"Syntax: {os.path.basename(sys.argv[0]).replace('.py', '')} [-m|--market=MARKETNAME] [-d|--node=NODE] ["
...
if __name__ == "__main__":
    ...
    for arg in sys.argv[1:]:
        ...
        if token in ["-m", "--market", "market"]:
            ...
            print('ISONE API requires credentials. If you do not have them, please apply for access at: \n '
    ...
    if None not in (market, node, start_date, end_date):
        get_market_data(market=market, node=node, start_date=start_date, end_date=end_date)
    else:
        error('market (-m), node (-d), start date (-s), and end date (-e) are required parameters')
        if market is None:
            error('Markets can be: \'caiso\', \'isone\'', code=1)
        elif node is None:
            error('User must indicate a node. For a list of CAISO nodes, download the latest \'Full Network Model '
                  'Pricing Node Mapping\' from the CAISO website. For a list of ISONE nodes, downloaded the latest '
                  '\'PNODE Table\' from the ISONE website.', code=1)
