...
brew install openssl ossp-uuid gdbm sqlite3 xz

# Install the proper version of python3
echo "Updating python3..."
PREFIX=/usr/local/opt/python@3.9
mkdir -p ${PREFIX%/*}   
if [ ! -d $PREFIX ]; then
    cd ${PREFIX%/*} 
    curl -sL http://install.gridlabd.us/python39-darwin19.tarz | tar xz
fi
if [ ! -d $PREFIX ]; then
    ...
    cd Python-3.9.6
    ./configure --silent --prefix=$PREFIX --enable-optimizations --with-computed-gotos --enable-loadable-sqlite-extensions CFLAGS="-fPIC" CPPFLAGS="-I$(brew --prefix openssl)/include" LDFLAGS="$(brew --prefix openssl)/lib"
